...
counts <- function(inbam, region, progress, filterin = 0, filterex = 1799, resolution, pos, onlymulti = F){
    ...
    if(onlymulti){
        ...
        paste("./get_contacts_from_bam_onlymulti.sh",
...
wrap <- function(inbam, region, resolution, filterin, filterex, onlymulti, progress,
                 wei_total = F){
    ...
    size <- paste("samtools view -H", inbam[1]) %>%
        ...
        mutate(V2 = gsub("^SN:", "", V2),
               V3 = gsub("^LN:", "", V3)) %>%
        filter(V2 == gsub(":.*$", "", region)) %$%
        V3 %>%
        as.numeric

    coord <- gsub("^.*:", "", region) %>% strsplit("-") %>% unlist %>% as.numeric
    ...
    if(length(coord) == 1){
        ...
        pos <- seq(floor(coord[1] / resolution) * resolution,
    ...
    dat <- parallel::mclapply(inbam, counts,
                              region = region, filterin = filterin, filterex = filterex,
...
wrap_from_raw <- function(mat, pzero = .999, itermax = 1, maxdev = .1, wei_total = F){
    ...
    # scale to have avg. contacts per cell = 1

    mat_norm <- mat_norm / (sum(mat_norm, na.rm = T) / (dim(mat_norm) %>% prod))
...
get_bam_path <- function(sampleid){
    ...
    sapply(sampleid, function(x) paste0("/mnt/cluster_4DGenome_no_backup/data/hic/merged/",
                                        x) %>%
                                 list.files(patt = paste0("^", x, ".*bam$"),
                                            rec = TRUE, full = TRUE))
...
make_download_filename <- function(region, resolution){
    
        paste0(region, "_", resolution, "Kbp.tsv.gz")
}

# log10 and NA to infinite
...
null_plot <- function(msg){
    
    plot(0, axes = F, xlab = "", ylab = "", type = "n")
    text(1, 0, msg, cex = 2.5)
