...
tadbit_after_bam=$PROJECT/pipelines/$pipeline_version/scripts/tadbit_after_bam.py
#tadbit_after_bam_merge=$PROJECT/pipelines/$pipeline_version/scripts/tadbit_after_bam_merge.py
...
while read line ;do 
	...
	if [[ $n_replicates -lt $min_n_replicates ]]; then
		echo "$n_replicates is smaller than $min_n_replicates; exiting..."
		exit
	fi
	...
	if [[ `echo $bio_name |wc -l` -gt 1 ]]; then
		echo "sequencing replicates do not come from the same biological sample; exiting..."
		exit
	fi

	# output directory
	ODIR=$DATA/merged/$bio_name 
	mkdir -p $ODIR
	...
	job_name=${process}_${mode}_${bio_name}
	...
	# merge BAMs
	obam=$ODIR/${bio_name}_both_map_merged.bam
	obai=$ODIR/${bio_name}_both_map_merged.bam.bai		
	job_cmd_merge="$samtools merge -rf -@ $slots $obam $ibams"
	job_cmd_index="$samtools index $obam"

	# apply TADbit after merging BAM
	job_cmd_tadbit="\$python $tadbit_after_bam \
								$obam \
								$flag_excluded \
								$flag_included \
								$flag_perzero \
	...
	elif [[ $mode == "tadbit" || $mode == "full" ]]; then
		echo $job_cmd_tadbit
	fi
