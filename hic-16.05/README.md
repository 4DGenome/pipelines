...
- `io_metadata.sh` (which also uses `io_metadata.py`) writes/access data to/from the metadata database
- `4DGenome_metadata.db` stores metadata and metrics generated by the pipeline
- genome reference sequences (e.g. mm10_chr1-19XYM.fa)
...
## TO-DO list
- try using genome assembly sequence including chrUn contigs