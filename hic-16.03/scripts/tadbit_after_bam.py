...
def main():
    ...
    process_AB(dat_list[0], perc_zero, resolution_list[0], outdir)
    ...
    process_TAD(dat_list[1], perc_zero, resolution_list[1], ncpus, outdir)
...
def process_AB(hic_data, perc_zero, reso, outdir):
    ...
    ev1 = hic_data.find_compartments()

    ev1_file = outdir + 'ev1_%s.tsv' % nice(reso)

    out_ev1 = open(ev1_file, 'w')
    out_ev1.write('\n'.join([str(v) for v in ev1]))
    out_ev1.close()
...
def process_TAD(hic_data, perc_zero, reso, cpus, outdir):
