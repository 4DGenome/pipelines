#!/bin/bash
#$ -N TE_S_T_2016_03_29_align_and_process_hic-16.03
#$ -q short-sl65
#$ -l virtual_free=40G
#$ -l h_rt=6:00:00
#$ -o /users/project/4DGenome/pipelines/hic-16.03/job_out/TE_S_T_2016_03_29_align_and_process_hic-16.03_$JOB_ID.out
#$ -e /users/project/4DGenome/pipelines/hic-16.03/job_out/TE_S_T_2016_03_29_align_and_process_hic-16.03_$JOB_ID.err
#$ -j y
...
pipeline_name=hic
pipeline_version=16.03
pipeline_run_mode=align_and_process
io_mode=standard
...
path_job_file=/users/project/4DGenome/pipelines/hic-16.03/job_cmd/TE_S_T_2016_03_29_align_and_process_hic-16.03.sh
...
main() {
	...
	if [[ $pipeline_run_mode == 'full' ]]; then
		...
		processed_fastqs_quality_plots
		align_and_process
	...
	elif [[ $pipeline_run_mode == 'processed_fastqs_quality_plots' ]]; then processed_fastqs_quality_plots
	elif [[ $pipeline_run_mode == 'align_and_process' ]]; then align_and_process
...
# ========================================================================================
# Processed FASTQs quality plots
# ========================================================================================

processed_fastqs_quality_plots() {

	step="processed_fastqs_quality_plots"
	time0=$(date +"%s")
	...
	message_info $step "making quality plots of the processed FASTQ files... plots saved at $QUALITY_PLOTS"
 ...
 #  	percentage_ligated_sites_read2=`echo $returned_values |cut -f4 -d';'`
	# python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_metrics $sample_id 'PERCENTAGE_DANGLING_ENDS_READ1' $percentage_dangling_ends_read1
	# python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_metrics $sample_id 'PERCENTAGE_LIGATED_SITES_READ1' $percentage_ligated_sites_read1
	# python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_metrics $sample_id 'PERCENTAGE_DANGLING_ENDS_READ2' $percentage_dangling_ends_read2
	# python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_metrics $sample_id 'PERCENTAGE_LIGATED_SITES_READ2' $percentage_ligated_sites_read2
	# message_update_metadata $step 'PERCENTAGE_DANGLING_ENDS_READ1' $metadata
	# message_update_metadata $step 'PERCENTAGE_LIGATED_SITES_READ1' $metadata
	# message_update_metadata $step 'PERCENTAGE_DANGLING_ENDS_READ2' $metadata
	# message_update_metadata $step 'PERCENTAGE_LIGATED_SITES_READ2' $metadata
 
	# # Update metadata
	# plot_path1=`ls $QUALITY_PLOTS/*read1*`
	# plot_path2=`ls $QUALITY_PLOTS/*read2*`
	# md5sum_plot_path1=`md5sum $plot_path1 | sed "s/  /;/g"`
	# md5sum_plot_path2=`md5sum $plot_path2 | sed "s/  /;/g"`
 #  	python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_paths $sample_id 'READ1_FILLED_PROCESSED_READS_QUALITY' $md5sum_plot_path1
 #  	python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_paths $sample_id 'READ2_FILLED_PROCESSED_READS_QUALITY' $md5sum_plot_path2
 #  	message_update_metadata $step 'READ1_FILLED_PROCESSED_READS_QUALITY' $metadata
 #  	message_update_metadata $step 'READ2_FILLED_PROCESSED_READS_QUALITY' $metadata
...
align_and_process() {
	...
	echo $gem_index
	exit
	python $SCRIPTS/map_process_merge.py $PROJECT \
 			$SAMPLE \
 			$species \
 			$tadbit_assembly_version \
 			$read_length \
 			$read1_filled \
 			$read2_filled \
 			$restriction_enzyme \
			$fasta \
			$slots \
			$tadbit_frag_map > $log_mapping
	rm -fr $SAMPLE/mapped_reads/tmp_dir*
	rm -fr $SAMPLE/results/processed_reads/tmp*
	message_info $step "output saved in $log_mapping"

	# Update metadata
	md5sum_log_mapping=`md5sum $log_mapping | sed "s/  /;/g"`
	python $SCRIPTS/update_and_get_samples_metadata.py $metadata update_paths $sample_id 'LOG_MAPPING' $md5sum_log_mapping
	message_update_metadata $step 'LOG_MAPPING' $metadata
