...
server <- function(input, output, session, plot) {
    ...
    region <- eventReactive(input$update_plot + 1, input$range * 1e3)
    ...
    filename_raw <- reactive(make_download_filename(chrom(),
                                                    resolution()))
    content_raw <- function(filename) make_download_content(dat_raw(), filename, resolution())
